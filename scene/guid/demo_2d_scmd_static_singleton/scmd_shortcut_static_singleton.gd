...
class_name SCmdShortcutStaticSingletonBridge
extends Node

static var _callable_listeners_shortcut_to_interpret: Array[Callable] = []


static func add_listener_of_shortcut_to_interpret(listener: Callable) -> void:
	if listener in _callable_listeners_shortcut_to_interpret:
		return
	_callable_listeners_shortcut_to_interpret.append(listener)

static func remove_listener_of_shortcut_to_interpret(listener: Callable) -> void:
	if listener in _callable_listeners_shortcut_to_interpret:
